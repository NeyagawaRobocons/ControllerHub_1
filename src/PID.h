...
private:
    ...
    pid_param_t gain;
public:
    PID(float k_i, float k_p, float k_d, float first_feedback, float first_target);
...
PID::PID(float k_i, float k_p, float k_d, float first_feedback = 0, float first_target = 0) : timer()
{
    this->gain.k_i = k_i;
    this->gain.k_p = k_p;
    this->gain.k_d = k_d;
